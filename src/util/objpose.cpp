...
inline Eigen::Vector3d optimal_t( const Eigen::MatrixXd& mP3DCentered,
                                  const std::vector<Eigen::Matrix3d>& vV,
...
int objpose( const Eigen::MatrixXd& mP3DNotC, ///<Input: 3xN matrix representing the landmarks in front of the camera
    ...
    // V_j = 
    std::vector<Eigen::Matrix3d> vV; vV.reserve( nNumPoints );
    Eigen::Vector3d vN;
    ...
    if( mMeas.rows() == 2 ) {
        ...
        std::cerr << "ERROR: in objpose, measurement matrix has wrong number of rows: " << mMeas.rows() << std::endl;
        return -1;
    }
	...
	double errorUpdate = (dOldError - dNewError )/dOldError;

    while( nNumIterations < nMaxNumIters &&
           abs( errorUpdate ) > dTol &&
           dNewError > dEpsilon ) {
        dOldError = dNewError;
		...
		errorUpdate = (dOldError - dNewError )/dOldError;
        ++nNumIterations;
    }
